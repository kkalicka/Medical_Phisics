...
def find_x_max(data):
    max = 0
    max_x = 0
    for i in range(120, len(data)-9):
        if (data[i]+data[i+1]+data[i+2]+data[i+3]+data[i+4])/5>max:
            max = data[i+2]
            max_x =i+2
    #return max_x*0.5 + 380
    return max
...
for file_name in file_names:
    ...
    if int(file_name[4:6]) >3:
        ...
        # usrednianie pomiarow, w tym przypadku w stosunku do 5 najbliższych pomiarów
        for i in range(0, len(data_y)-5):
            data_y[i+2] = (data_y[i]+data_y[i+1]+data_y[i+2]+data_y[i+3]+data_y[i+4])/5

        if len(data_tlo)>0:
            for i in range(0, len(data_y)):
                if data_tlo[i]>data_y[i]:
                    data_y[i] = data_y[i] - data_tlo[i]
        max_data_y=max(data_y[100:300])


        print(find_x_max(data_y))
        plt.xlim([450, 650])
        plt.ylim([0, 550])
